...
- 状态枚举与 P2 表结构已出现落地痕迹，但还缺业务链路证据：
  - `BidProjectStatusEnum` 已有 `OPENING_IN_PROGRESS`、`EVALUATING`、`AWARDED`
  - `BidLotStatusEnum` 已有 `OPENED`、`EVALUATING`、`AWARDED`
  - `BidSubmissionStatusEnum` 已有 `OPENED`，评标 / 定标结果仍主要通过 opening / evaluation / award 表承接
  - `v3.36.0.sql`、opening / evaluation / award 后端模块和管理端页面已存在，下一步应验证状态推进和 workflow history，而不是重复扩展枚举
...
当前代码中 `BidProjectStatusEnum` 已增加：
